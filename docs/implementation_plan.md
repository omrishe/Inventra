...
* **Silent Refresh on Mount:** `App.tsx` fires `POST /api/v1/auth/refresh` on startup. If the cookie is valid, the Zustand store is populated and the user lands on their last page. If the cookie is missing or expired, the user is shown the login page.
